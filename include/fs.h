...
struct inode {
  int id;
  short int type;  // dir or file
  short int nlink;
  int device;  //
  int size;
  int blocks[INODEBLOCKS]; // first 12 blocks which contain file data
};
...
struct fsystem {
  ...
  int inodes_used;  
  int freemaskbytes; //length of freemask in bytes
  char *freemask;  // a bit indicates a block's used1/empty0
